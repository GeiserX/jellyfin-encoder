...
- A video renamed inside the source tree, or a renamed folder, is now handled within one
  ...
  moves with it; 1.4.0 and 1.5.x re-encoded it under the new name.
...
and nothing is encoded again, so renaming a whole folder costs a rename per file. Only a
source whose encode is missing, or still being written, is encoded under the new name. A
file copied in from outside is a plain create and is handled the same way.
